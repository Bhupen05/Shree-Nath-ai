...
function BillingPage() {
  ...
  const calculateTotals = () => {
    const qty = Number(draftForm.quantity || 1)
    const unitPrice = Number(draftForm.unitPrice || 0)
    const taxRate = Number(draftForm.taxRate || 18) / 100
  ...
  const submitDraft = async (event) => {
    ...
    if (
      !draftForm.partyId ||
      !draftForm.partId ||
      Number(draftForm.quantity) <= 0 ||
      Number(draftForm.unitPrice) <= 0
    ) {
    ...
    try {
      const payload = {
        ...
        partyType: draftForm.billType === 'SALE' ? 'CUSTOMER' : 'SUPPLIER',
  ...
  if (loading) {
    return (
      <StatusView
        ...
        message="Collecting invoices and payment states..."
      />
    )
  }
    ...
    <section className="stack">
      ...
      {canWrite && (
        <form className="inline-form" onSubmit={submitDraft}>
          ...
          <div className="inline-grid">
            ...
            <label className="form-field" htmlFor="partyId">
              ...
              <select
                ...
                onChange={(event) =>
                  setDraftForm((prev) => ({ ...prev, partyId: event.target.value }))
                }
              >
            ...
            <label className="form-field" htmlFor="partId">
              ...
              <select
                ...
                onChange={(event) =>
                  setDraftForm((prev) => ({ ...prev, partId: event.target.value }))
                }
              >
                <option value="">Select</option>
                {parts.map((part) => (
            ...
            <FormField
              ...
              onChange={(event) =>
                setDraftForm((prev) => ({ ...prev, quantity: event.target.value }))
              }
            />
            ...
            <FormField
              ...
              onChange={(event) =>
                setDraftForm((prev) => ({ ...prev, unitPrice: event.target.value }))
              }
            />
            ...
            <FormField
              ...
              onChange={(event) =>
                setDraftForm((prev) => ({ ...prev, taxRate: event.target.value }))
              }
            />
          </div>
          ...
          >
            ...
            <p>
              <strong>Tax ({draftForm.taxRate}%):</strong> ₹{totals.tax.toFixed(2)}
            </p>
          ...
          <div className="inline-grid">
            <label className="form-field" htmlFor="paymentBillId">
              ...
              <select
                ...
                onChange={(event) =>
                  setPaymentForm((prev) => ({ ...prev, billId: event.target.value }))
                }
              >
                <option value="">Select</option>
                {items
                  .filter((bill) =>
                    ['CONFIRMED', 'PARTIALLY_PAID'].includes(bill.status)
                  )
                  .map((bill) => (
                    <option key={bill.id} value={bill.id}>
                      {bill.bill_number} ({bill.status}) - Due: ₹
      ...
      {items.length === 0 ? (
        <StatusView
          mode="empty"
          title="No bills yet"
          message="As soon as bills are created, they will appear here."
        />
      ) : (
        <DataTable
          title="Recent bills"
          ...
          columns={[
            ...
            },
            {
              ...
              label: 'Due (₹)',
              sortable: true,
              render: (row) => `₹${Number(row.amount_due || 0).toFixed(2)}`,
            },
            {
              key: 'created_at',
              label: 'Created',
            ...
            },
            {
              ...
              render: (row) => (
                <div className="row-actions">
                  ...
                  {canWrite &&
                    ['DRAFT', 'CONFIRMED', 'PARTIALLY_PAID'].includes(
                      row.status
                    ) && (
                    <button
                      onClick={() =>
      ...
      {selectedBillDetail && (
        <section className="inline-form">
          ...
          <p className="sub">
            Type: {selectedBillDetail.bill.bill_type} | Status:{' '}
            {selectedBillDetail.bill.status}
          </p>
          <p className="sub">
            Subtotal: ₹{Number(selectedBillDetail.bill.subtotal || 0).toFixed(2)} | Tax:
            ₹{Number(selectedBillDetail.bill.tax || 0).toFixed(2)} | Total: ₹
            {Number(selectedBillDetail.bill.total || 0).toFixed(2)}
          </p>
          <p className="sub">
            Paid: ₹{Number(selectedBillDetail.bill.amount_paid || 0).toFixed(2)} |
            Due: ₹{Number(selectedBillDetail.bill.amount_due || 0).toFixed(2)}
          </p>
          <div className="detail-grid">
            <div>
              <h3>Items</h3>
              {selectedBillDetail.items.length === 0 ? (
                <p>No items.</p>
              ) : (
            ...
            <div>
              ...
              {selectedBillDetail.payments.length === 0 ? (
                ...
                selectedBillDetail.payments.map((payment) => (
                  <p key={payment.id}>
                    ₹{Number(payment.amount || 0).toFixed(2)} via{' '}
                    {payment.payment_mode} on{' '}
          ...
          <div className="inline-actions">
            <Button onClick={() => setSelectedBillDetail(null)}>Close</Button>
          </div>
        </section>
      )}
    </section>
